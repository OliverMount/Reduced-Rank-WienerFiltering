...
clear all;clc;tic;
noiter=50; Z=4; len=1000;

SNR=0:2:25;  Es=1; No=Es./10.^(SNR/10); 
codebook=QAM_codebook(Z);
...
% % Ideal value
% Psc=2*(1-(1/sqrt(Z)))*Q(3*SNR/(Z-1));
% Pc=1-((1-Psc).^2);

% figure,hnd=semilogy(SNR,ensemble_avg,'k',SNR,Pc,'b');grid;
% legend('Theoretical','Practical');
% axis([min(SNR) max(SNR) 10e-6 1]);

figure,hnd=semilogy(SNR,ensemble_avg,SNR,ensemble_avg/log2(Z));grid;
legend('SER','BER');

hnd=title('Coherent 16-QAM Performance Chart');
set(hnd,'fontsize',15);
...
% hnd=scatter(real(codebook),imag(codebook)); set(hnd,'linewidth',2);grid;