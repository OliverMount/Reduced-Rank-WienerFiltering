...
Eb=1; EbNodB=0:2:14; No=1./10.^(EbNodB/10);  Es=log2(Z)*Eb;
...
for loop=1:noiter,
 ...
 for i=1:length(No),
   for q=1:O, % upto  no of OFDM symbols
    ...
    [H_capt,H_capt_RR]=KTFt_esti(y,Pilot,M,D); E_capt=diag(H_capt);% Time doamin TF estimator
%     Y=F*y;
%     H_capf=TFf_esti(Y,Pilot);  E_capf=diag(H_capf);% Freq doamin TF estimator
    E_capt_RR1=diag(H_capt_RR(:,1));% Time doamin TF estimator
    E=F*H*F';
...
Y_decoded_capt_RR1(:,q)=E_capt_RR1'*inv(E_capt_RR1*E_capt_RR1'+(No(i)/var(X_s))*eye(N))*Y_p(:,q);
    end
    Y=PS(Y_decoded,N); % Parallel to serial conversion (with known channel) 
    Y_capt=PS(Y_decoded_capt,N); % With estiamted channel
%     Y_capf=PS(Y_decoded_capf,N); % With estiamted channel
...
%      pe_capf(loop,i)=(len-sum(number2))/len;
    ...
    number_rr1=(decoded_capt_rr1==X_s);
...
legend('Theoretical','Perfect Channel','Time-domain Estimator','Time-domain Rank-1');
axis([0 16 10e-8 0.5]);
