...
startr=4;endr=2;
del=25;  % Decision delay (From del=5 started getting results) (Note that decision dely could be longer than equlaizer length)
M=21;  % Number of Taps of the equalizer
% D=5; % Dimension of the Subspace
p=M; % Full rank Dimension

 Eb=1; EbNodB=0:18; No=1./10.^(EbNodB/10);  Es=log2(Z)*Eb;
 codebook=sqrt(Es)*exp(j*2*pi*(0:Z-1)/Z + j*pi/4);
 len_no=length(No);
 len_r=startr-endr+1;
...
for loop=1:noiter,
    ...
    for i=1:length(No),
        ...
        X=var(TRANSseq)*toeplitz(h_ori(1:M));  %Transmitted correlation
