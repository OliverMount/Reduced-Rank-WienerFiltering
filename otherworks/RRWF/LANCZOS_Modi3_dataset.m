...
k=200;
nois=randn(1,k);
d=AutoRnoise([1  -.6 -.3]',nois,k); % Desired signal
...
figure,plot(1:k,d,'r',1:k,recovered_signal);
legend('Original','Estimated');
...
for i=1:p,
    T=V(:,1:i); % eigen vectors
    lamda(i)=D(i,i); % Eigen values
    I(i,i)=lamda(i);
    Trans_auto=T'*Rv2*T;
    Trans_cross=T'*temp;
    eig_wopt{1,i}=inv(Trans_auto)*Trans_cross; % Optimum solution
    conv_d=conv_rv2*T;
    estimated_signal(i,:)=(conv_d*eig_wopt{1,i})';
    desired_signal_eigen(i,:)=x-estimated_signal(i,1:k);
    MSE_eig(i)=rv1(1)-temp'*T*inv(I)*T'*temp;
end
...
% figure,plot(1:k,recovered_signal,1:k,desired_signal_eigen(6,:),1:k,desired_signal_CS(6,:));
...
% hnd=title('');
% set(hnd,'fontsize',15);
hnd=xlabel('Sample Index');set(hnd,'fontsize',13,'color',[0 0 1],'color',[0 0 1]);
hnd=ylabel('Amplitude');set(hnd,'fontsize',13,'color',[0 0 1]);

str1=num2str(MSE); str2=num2str(MSE_RR(6));str3=num2str(MSE_CS(6));
str1=strcat('Full rank Wiener MSE :   ',str1);
str2=strcat('MSNWF MSE :   ',str2);
str3=strcat('CS MSE :   ',str3);
...
% set(hnd,'fontsize',15);
hnd=xlabel('Rank');set(hnd,'fontsize',13,'color',[0 0 1]);
hnd=ylabel('MSE');set(hnd,'fontsize',13,'color',[0 0 1]);

% str1=num2str(MSE); str2=num2str(MSE_RR(6));str3=num2str(MSE_CS(6));
% str4=num2str(MSE_eig
% str1=strcat('Full rank Wiener :  ' ,str1);
% str2=strcat('MSNWF  :  ' ,str2);
% str3=strcat('CS  :  ' ,str3);
% str4=strcat('PC  :  ',str4);
% legend(str1,str2,str3);

legend('Full rank','PC','CS','MSNWF');


% Choise of the Order of the Filter


desired_point=100;
for newvar=1:desired_point, 
    Rv2=toeplitz(rv2(1:newvar)); 
    w_opt=Levinson(rv2(1:newvar),rxv2(1:newvar)); % finding the optimal solution
    MSE_new(newvar)=rv1(1)-w_opt'*Rv2*w_opt;
end

figure,plot(MSE_new);


% Full rank MSE 0.161437 
% Eigen MSE 
 
%     2.1063    2.0316    1.3059    0.7259    0.5783    0.2992    0.2373    0.1797    0.1790    0.1790
%     0.1625    0.1614
 
% CS Metric MSE 

%     2.0140    1.3807    0.8006    0.5215    0.3739    0.2992    0.2373    0.1797    0.1632    0.1621
%     0.1614    0.1614

% MSNWF MSE 
  
%     1.0351    0.3642    0.2586    0.1793    0.1631    0.1618    0.1615    0.1614    0.1614    0.1614
%     0.1614    0.1614