...
% This program is just a testing in the AWGN Channel
% RR_QPSK
clear all;clc;tic;
noiter=25; Z=4; len=1000;
startr=4;endr=2;
del=0;  % Decision delay (From del=5 started getting results) (Note that decision dely could be longer than equlaizer length)
M=21;  % Number of Taps of the equalizer
...
for loop=1:noiter,
    ...
    for i=1:length(No),
        ...
        X=var(TRANSseq)*toeplitz(h_ori(1:M));  % variance 2 transmitted correlation
...
% for D=startr:-1:endr,
% ensemble_avg_RR(D,:)=mean(pe_RR{1,D});
% end
...
figure,hnd=semilogy(EBNO,erfc(sqrt((Eb./No))).*(1-.25*erfc(sqrt((Eb./No)))),'k-',EBNO,ensemble_avg);
...
legend('NO ISI','Full Rank'); % Only for testing
axis([0 13 10e-7 1]);
% 
% hnd=xlabel(' Eb/No  in   dB ');
...
% Nice result
% Results for D=4:-1:2 (N= 21 tap equalizer delay 25) ( 4 3 2 graph is good
% although we have result for rank 5 also)
% For 400 iteration with 10000 bits
% ensemble_avg =
% 
%   Columns 1 through 11 
% 
%     0.2070    0.1589    0.1158    0.0796    0.0512    0.0306    0.0169    0.0085    0.0039    0.0016    0.0006
% 
...
%     0.2109    0.1640    0.1219    0.0863    0.0580    0.0370    0.0223    0.0127    0.0068    0.0035    0.0018
%     0.2074    0.1595    0.1165    0.0804    0.0521    0.0315    0.0176    0.0090    0.0042    0.0018    0.0007
%     0.2070    0.1589    0.1159    0.0797    0.0512    0.0308    0.0170    0.0086    0.0039    0.0016    0.0007
% 
%   Columns 12 through 15 
% 
%          0         0         0         0
