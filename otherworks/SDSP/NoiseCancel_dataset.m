...
o=6; % desired order in CS
D_temp=zeros(o,o);
M=nchoosek(p,o);
combi=nchoosek(1:12,o);
...
desired_signal_CS=x-estimated_signal_CS(1:k);
    
MSE_CS=rv1(1)-temp'*T*inv(lamda_CS)*T'*temp;

figure,plot(1:k,recovered_signal,1:k,desired_signal_eigen(6,:),1:k,desired_signal_CS);
legend('Full rank Wiener','Eigen Decomposition (6)','CS (6)'); 

% Original ,ED ,CS
figure,plot(1:k,d,1:k,desired_signal_eigen(6,:),1:k,desired_signal_CS);
legend('Original','Eigen Decomposition (6)','CS (6)'); 
% Original ,FRW ,CS
figure,plot(1:k,d,1:k,recovered_signal,1:k,desired_signal_CS);
legend('Original','Full rank Wiener','CS (6)'); 
 disp(MSE);disp(MSE_eig(1,6));disp(MSE_CS);
...
figure,plot(1:k,recovered_signal,'r',1:k,recovered_signal_RR(6,:),'k*',1:k,desired_signal_CS,'b*');
hnd=title('Full rank Wiener And MSNWF ,CF ( r = 6 ) Comparison');
set(hnd,'fontsize',15,'color',[0 0 1]);
hnd=xlabel('Sample Index');set(hnd,'fontsize',15);
hnd=ylabel('Amplitude');set(hnd,'fontsize',15);
str1=num2str(MSE); str2=num2str(MSE_RR(6));str3=num2str(MSE_CS);
