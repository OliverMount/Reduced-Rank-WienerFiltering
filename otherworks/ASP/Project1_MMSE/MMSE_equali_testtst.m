...
tic;
noiter=200; % number of averaing iterations
len=1000; % length of the transmiited sequence


del=16;  % Decision delay
M=31;    %MMSE filter length 
...
% h=[.407 .815 .407];
h=[.04 -.05 .07 -.21 -.5 .72 .36 .21 .03 .07];
% h=[.227 .460 .668 .460 .227];
% h=[.5 1.2 1.5 -1];
% h=[1 1.2 1.5 -1];
% W=2.9;
% for n=1:3,
...
for loop=1:noiter,   
    ...
    r_dx=r_dx_mtx(del(del_i)+1,:);   % Change here the decision variable
        ...
        for i=1:length(noisevar),
            ...
            rx = s > 0;  % Decision Box 
            rx1=(2*rx-1);
            rx_all=rx1(del(del_i)+1:len+del(del_i)); % L= ISI length
...
pe_del(del_i,:)=mean(pe);

end

No=noisevar;
EBNO=10*log10(Eb./No);
% EBNO=10*log10(sum(h)./No);

figure,hnd=semilogy(EBNO,.5*erfc(sqrt((Eb./No))),'b',EBNO,(pe_del(1,:)),EBNO,(pe_del(1,:)),EBNO,(pe_del(1,:)),EBNO,(pe_del(1,:)),EBNO,(pe_del(1,:)));grid;
