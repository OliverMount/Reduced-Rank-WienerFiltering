...
h=[.227 .460 .668 .460 .227]; % Proaksi Channel C (Spectral Nulls)
% h=[.5 1.2 1.5 -1];
% h=[1 1.2 1.5 -1];
% W=2.9;
% for n=1:3,
...
for loop=1:noiter,   
    ...
    r_dx=r_dx_mtx(del+1,:);   % Change here the decision variable
        ...
        for i=1:length(noisevar),
            ...
            rx = s > 0;  % Decision Box 
            rx1=(2*rx-1);
            rx_all=rx1(del+1:len+del); % L= ISI length
...
figure,hnd=semilogy(EBNO,.5*erfc(sqrt((Eb./No))),'b',EBNO,(ensemble_avg),'r');grid;
